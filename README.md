...
| **SonarQube** | 9.9.x | Static Code Analysis |
| **SonarScanner CLI** | 4.8.1 | SonarQube Scanner |
...
### SonarQube
1. Run SonarQube 9.9.x server
2. Create a project with key `My-website`
3. Generate an auth token → add to Jenkins as `Sonar-token`
4. Add SonarQube server in Jenkins → `Manage Jenkins` → `Configure System` → name it `sonar-server`
5. Add SonarScanner tool in Jenkins → `Manage Jenkins` → `Global Tool Configuration` → name it `sonar-scanner`

### AWS ECR
Ensure the ECR repository exists:

```bash
aws ecr create-repository --repository-name nhytdfjynrsnydt --region ap-south-1
```
...
GitHub: [@Vipin-Patel-devops](https://github.com/Vipin-Patel-devops)
